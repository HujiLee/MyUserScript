...
// @grant none
// @include     *://www.tsdm39.*/forum.php?mod=viewthread&tid=*
// @run-at      document-end
// ==/UserScript==
...
if (!document.querySelector("#postlist>[id^=post_]")) {
  ...
  var size = (function () {
    ...
    jQuery(title).find("span").remove();//20210428更新
  ...
  var goJinghua = function (level) {
    ...
    if (!A_TAG_Named_Jinghua) {
      console.log("本帖没有加精华权限")//20210428
      return parent.jQuery(parent).trigger("JHERROR");
    }
  ...
  if (localStorage["JH" + tid]) {
    ...
    console.log("localStorage中不存在：", "JH" + tid);//20210428
    parent.jQuery(parent).trigger("JHERROR");
  }
